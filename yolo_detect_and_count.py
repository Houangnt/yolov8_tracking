...
class YOLOv8_ObjectDetector:
    def __init__(self, model_file = 'yolov8n.pt', labels= None, classes = None, conf = 0.35, iou = 0.45 ):
    ...
    def custom_display(self, colors, show_cls = True, show_conf = True):
        ...
        for box in self.results.boxes:
            ...
            img = cv2.rectangle(img, (x1, y1), (x2, y2), colors[min(class_id, len(colors) - 1)], bbx_thickness)
            center_coordinates = ((x1 + x2)//2, (y1 + y2) // 2)
            img = cv2.circle(img, center_coordinates, 5, (0, 0, 255), -1)

            # Draw filled rectangle for details text
            if textString != "":
                if (y1 < textSize[1]):
                    y1 = y1 + textSize[1]
                else:
                    y1 -= 2
                img = cv2.rectangle(img, (x1, y1), (x1 + textSize[0], y1 - textSize[1]), colors[min(class_id, len(colors) - 1)], cv2.FILLED)
...
class YOLOv8_ObjectCounter(YOLOv8_ObjectDetector):
    ...
    Returns:
        ...
        while cap.isOpened():
            ...
            classCounts = {}
            allClassIDs = list(range(len(self.labels)))
            ...
            if display == 'default':
                ...
                # Pass the required 'colors' argument to custom_display
                frame = self.custom_display(colors=[(255, 0, 0)], **display_args)
            vertical_position = 45
            # Display Counting results
            for class_id in allClassIDs:
                class_name = self.labels[class_id]
                count = classCounts.get(class_id, 0)
                count_txt = f"Class {class_name} - total count: {count}"
                frame = cv2.putText(frame, count_txt, (5, vertical_position), cv2.FONT_HERSHEY_COMPLEX, 2, (0, 0, 255), 2)
...
yolo_counter = YOLOv8_ObjectCounter(model_file='car_detection.pt')
video_path = 'test1.mp4'
output_dir = 'output_videos'
yolo_counter.predict_video(video_path=video_path, save_dir=output_dir, save_format='avi', display='custom')d